                    ...
                    </Grid>
                    {
                        this.state.results.map((item, index) => (
                            <Grid item xs={12} md={4} lg={3} xl={3}>
                                <CustomCard deleteFunction={this.deleteItem} functionToCall={this.loadExperiments} passThis={this} link={item.link} id={item.id} title={item.algorithmName} content={item.description} href={"/show/result/" + item.algorithmName.replace(/\s/g, "").toLowerCase() + "/" + item.id} date={item.modificationDate} removeIcon={true}/>
                            </Grid>
                        ))
                    }
                </Grid>
